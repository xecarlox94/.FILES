{ config, pkgs, ... }:
{
  ...
  home = {
    ...
    packages = with pkgs; [
      ...
      (pkgs.writeShellScriptBin "add_path" ''
          export PATH="$PATH:$1"
        ''
      )


      (pkgs.writeShellScriptBin "mkdir_if_not_exists" ''
          ! [ -d $1 ] \
          && mkdir -p $1
        ''
      )
      ...
      (pkgs.writeShellScriptBin "touch_if_not_exists" ''
          ! [ -f "$1" ] \
            && mkdir -p $(dirname $1) \
            && touch $1
        ''
  ...
  programs = {
    ...
    bash = {
      ...
      bashrcExtra = ''
        ...
        add_path $HOME/.config/.FILES/nix/.config/home-manager/deprecating_scripts
